...
\author{
David Schruth\\ \texttt{dschruth@uw.edu} \and Adrian Marchetti \\ \texttt{amarchetti@unc.edu}
}
\maketitle
\section{Licensing}
...
For the purposes of example we use \Rpackage{edgeR}'s \Rfunction{estimateGLMCommonDisp} which returns a common dispersion estimate near 0.9.
<<dispersion2manual>>=
...
@
\end{center}
\caption{an example of de2ra(exactTest(obj)) call.
}
\label{fig:three}
\end{figure}
\newpage
